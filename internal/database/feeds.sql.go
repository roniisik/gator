...
const getUserIDName = `-- name: GetUserIDName :one
SELECT name FROM users WHERE id = $1
`

func (q *Queries) GetUserIDName(ctx context.Context, id uuid.UUID) (string, error) {
	row := q.db.QueryRowContext(ctx, getUserIDName, id)
	var name string
	err := row.Scan(&name)
	return name, err
}
